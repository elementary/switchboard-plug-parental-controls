...
/*-
 * Copyright (c) 2012-2014 Granite Developers (http://launchpad.net/granite)
...
namespace PC.Widgets {
    public class EmbeddedAlert : Gtk.EventBox {
        ...
        private const int MIN_HORIZONTAL_MARGIN = 84;
        private const int MIN_VERTICAL_MARGIN = 48;
        ...
        public EmbeddedAlert () {
            ...
            link_button = new Gtk.LinkButton.with_label ("", _("Add users to begin editing"));
            link_button.tooltip_text = _("Open Users settings");
            link_button.activate_link.connect (() => {
                var cmd = new Granite.Services.SimpleCommand ("/usr/bin", "switchboard user-accounts");
                cmd.run ();
                return true;
            });

            link_button.valign = link_button.halign = Gtk.Align.START;
            ...
            message_vbox.pack_start (link_button, false, false, 0);
        ...
        private Gtk.Button? new_button_from_action (Gtk.Action action) {
            if (action == null)
                return null;
